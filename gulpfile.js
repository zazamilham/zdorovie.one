...
function styles_theme() {
    return src(["app/assets/src/scss/**/*.scss"])
...
function deploy() {
    ...
    return src(globs, { base: "./app", buffer: false })
        .pipe(
            conn.newer("/zdorovie.one/public_html/wp-content/themes/zdravmat2")
        )
        .pipe(
            conn.dest("/zdorovie.one/public_html/wp-content/themes/zdravmat2")
        );
}
...
function watching() {
    watch(
        ["app/assets/src/scss/**/*.scss"],
        series(styles_vendor, styles_theme)
    );
